...
- Work (bug fix — audio playing twice / audio not stopping after pause): Both the desktop 3-up grid and the mobile 1-card carousel were rendering in the DOM simultaneously — Tailwind `hidden md:grid` / `md:hidden` set `display:none`, which does NOT stop iframe autoplay audio. TWO iframes with `autoplay=1` were mounting for the same story so audio played in parallel; pausing the visible one left the hidden one running. FIX: an `isMobile` state seeded synchronously from `window.matchMedia("(max-width: 767px)")` in Work.jsx — the desktop grid renders only when `!isMobile`, the mobile carousel only when `isMobile`, and any layout change also resets `playingIdx` to -1 so no orphan iframe survives a resize. Only ONE `<iframe>` can now exist in the DOM. Verified across desktop + mobile + cross-viewport switches by the testing agent.

...
test_plan:
  current_focus:
    - "Approach 'Push / Pull' Examples card must sit INSIDE the LG monitor bezel at every desktop viewport width (1280, 1366, 1440, 1600, 1920). The card must not overflow past the monitor screen edges (previously the axis-aligned fallback rectangle was TALLER than the SCREEN_QUAD, so when the CSS matrix3d homography failed to compute on initial mount the card leaked past the bottom bezel — see user screenshot). Verify: (1) the card's outer bounds are always contained inside the black monitor screen region; (2) the 'EXAMPLES' label at top-left, 'MADE TO BROADCAST' at top-right, and 'Every brand is stuck here' at the bottom all sit WITHIN the monitor's screen area, never on the bezel or the desk. Toggle both PULL and PUSH modes and verify in both."
  stuck_tasks: []
  test_all: false
